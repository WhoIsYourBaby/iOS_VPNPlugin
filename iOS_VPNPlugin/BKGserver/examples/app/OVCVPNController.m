...
#define printf(fmt, ...)           CFLog(1, CFSTR("<%s> "fmt), __FUNCTION__, ##__VA_ARGS__) 

#define OPENVPN_PLUGIN_TYPE     CFSTR("com.secworld.bkgserver")
...
static void confCallback(VPNConfigurationRef conf, unsigned int notification, void* info) {
    ...
    switch (notification) {
        case 0: {
            ...
            /*
                printf("Got a VPN connection status changed event from the framework: notification: %p, status: %d info: %p (%@)", 
        ...
        case 1: // Got a VPN configuration changed event from the framework
            ...
            printf("Got a VPN configuration changed event from the framework: notification: %p, info: %p (%@)", 
                notification, info, info);
            break;
            
        default:
            printf("conf: %@, notification: %p, info: %p (%@)", 
...
static void msgCallback(VPNConfigurationRef conf, unsigned int message, CFDataRef data, void* info) {
    //printf("conf: %@, message: %d, data %@", conf, message, data);
...
- (void) doPendingConnect {
    ...
    if (!enabled) {
        printf("VPNConfigurationConnectionStart() failed: %@", VPNCopyLastError());
        return;
    } 
...
- (void) processMessage:(uint32_t)message withDictionary:(NSDictionary*)dict {
    switch (message) {
        ...
        default:
            printf("message: %d, data %@", message, dict);
            break;
    }
}
...
- (void) processMessage:(uint32_t)message withData:(CFDataRef) data {
    ...
    if (dict == nil) {
        printf("CFPropertyListCreateWithData(): %@", error);
        CFRelease(error);
        return;
    }
...
- (void) sendMessage:(int32_t)type withDictionary:(NSDictionary*)dict {
    VPNConfigurationRef conf = [self enabledConfig];
    if (conf == nil) {
        printf("No enabled config found");
        return;
    }
...
- (void) sendMessage:(int32_t)type withDictionary:(NSDictionary*)dict toConfig:(VPNConfigurationRef)conf {
    ...
    if (data == nil) {
        printf("CFPropertyListCreateData(): %@", error);
        CFRelease(error);
        return;
    }
    
    printf("VPNConfigurationSendMessage(%d, %@)", type, data);
    
    Boolean enabled = VPNConfigurationSendMessage(conf, type, data);
    if (!enabled)
        printf("VPNConfigurationSendMessage() failed: %@", VPNCopyLastError());
...
- (void) ackTimerFires:(NSTimer*) timer {
    ...
    if (++ _ackRetries > 10) {
        [self stopAckTimer];
        printf("Failed to attach vpn plugin");
        return;
    }
...
- (BOOL) enablePlugin {
    ...
    if (!enabled) {        
        printf("VPNConfigurationEnableVPNType() failed: %@", VPNCopyLastError());
    }
...
- (void) copyConfigurations {
    ...
    //printf("VPNConfigurationCopyAll(): %@", confs);
        ...
        */
        ...
        for (i = 0; i < [_vpnConfigurations count]; i++) {
            ...
            if (c) {
                ...
                //printf("[%d] %@: %s (%d) %@", i, VPNConfigurationGetName(c), s, status, c);
            }
        }
    }
}
...
- (BOOL) stopConfig:(VPNConfigurationRef) conf {
    ...
    if (!enabled) {
        printf("VPNConfigurationConnectionStop() failed: %@", VPNCopyLastError());
        return NO;
    } 
