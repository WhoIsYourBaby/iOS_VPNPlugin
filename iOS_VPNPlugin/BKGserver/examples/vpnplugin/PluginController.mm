...
void PluginController::appendManagementBuffer(char* buf, int len) {
    ...
    if (len > mgmtStreamSize) {
        printf("ignore %s byte mgmt buffer", len);
        return;
    }
    
    if (mgmtStreamLen_ > len > mgmtStreamSize - 1) {
        printf("flush mgmtstream of %u bytes", mgmtStreamLen_);
        mgmtStreamLen_ = 0;
    }
...
//    RELEASE_CSTRING(s2);    
    ...
    NSBundle* bundle = [NSBundle bundleWithIdentifier:@"com.secworld.bkgserver"];
...
//    GET_CSTRING(data, s1);    
//    printf("app %d type %d data %s", app, type, s1);
//    RELEASE_CSTRING(s1);
